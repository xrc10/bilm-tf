...
def train(options, data, tf_save_dir, tf_log_dir,
    ...
    with tf.device('/cpu:0'):
        ...
        # calculate the gradients on GPU
        cross_lingual_loss = tf.get_variable(
            'cross_lingual_loss', [],
            initializer=tf.constant_initializer(0.0), trainable=False)

        with tf.device('/gpu:0'):
            model = CrossLingualLanguageModel(options, True)
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        loader = tf.train.Saver(model.src_lm_variables)
        loader.restore(sess, restart_ckpt_file)

        # load the checkpoint data if needed
        if restart_ckpt_file is not None:
            loader = tf.train.Saver(model.trainable_variables)
            loader.restore(sess, restart_ckpt_file)
        ...
        init_state_tensors = model.init_lstm_states
        final_state_tensors = model.final_lstm_states
        ...
        if not char_inputs:
            ...
            feed_dict = {
                model.src_tokens_characters:
                    np.zeros([batch_size, unroll_steps, max_chars],
                             dtype=np.int32),
                model.trg_tokens_characters:
                    np.zeros([batch_size, unroll_steps, max_chars],
                             dtype=np.int32),
            }
        ...
        if bidirectional:
            if not char_inputs:
                feed_dict.update({
                    model.src_token_ids_reverse:
                        np.zeros([batch_size, unroll_steps], dtype=np.int64),
                    model.trg_token_ids_reverse:
                        np.zeros([batch_size, unroll_steps], dtype=np.int64)
                })
            else:
                feed_dict.update({
                    model.src_tokens_characters_reverse:
                        np.zeros([batch_size, unroll_steps, max_chars],
                                 dtype=np.int32),
                    model.trg_tokens_characters_reverse:
                        np.zeros([batch_size, unroll_steps, max_chars],
                                 dtype=np.int32),
                })
        ...
        for batch_no, batch in enumerate(data_gen, start=1):
            ...
            # take the input in the batch for the feed_dict
            X = batch
            # use the last init states for lstm
            ...
            # This runs the train_op, cross_lingual_loss and the "final_state_tensors"
            #   which just returns the tensors, passing in the initial
            #   state tensors, token ids and next token ids
            if batch_no % 1250 != 0:
                ret = sess.run(
                    [train_op, cross_lingual_loss] +
                                                final_state_tensors,
                    feed_dict=feed_dict
                )
            ...
            if batch_no % 100 == 0:
                print("Batch %s, cross_lingual_loss=%s" % (batch_no, ret[1]))
