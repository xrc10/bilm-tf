...
from bilm_align.align_model import LanguageModel
...
def _get_feed_dict_from_X(X, Y, start, end, model, char_inputs, bidirectional):
    feed_dict = {}
    for k, data in enumerate([X, Y]):

        if not char_inputs:
            token_ids = data['token_ids'][start:end]
            feed_dict[model.token_ids[k]] = token_ids
        else:
            # character inputs
            char_ids = data['tokens_characters'][start:end]
            feed_dict[model.tokens_characters[k]] = char_ids

        if bidirectional:
            if not char_inputs:
                feed_dict[model.token_ids_reverse[k]] = \
                    data['token_ids_reverse'][start:end]
            else:
                feed_dict[model.tokens_characters_reverse[k]] = \
                    data['tokens_characters_reverse'][start:end]

        # now the targets with weights
        next_id_placeholders = [[model.next_token_id[k], '']]
        if bidirectional:
            next_id_placeholders.append([model.next_token_id_reverse[k], '_reverse'])

        for id_placeholder, suffix in next_id_placeholders:
            name = 'next_token_id' + suffix
            feed_dict[id_placeholder] = data[name][start:end]

    return feed_dict


def train(options, src_data, trg_data, n_gpus, tf_save_dir, tf_log_dir,
    ...
    with tf.Session(config=tf.ConfigProto(
        ...
        if not char_inputs:
            ...
            for k in [0, 1]:
                for model in models:
                    feed_dict[model.token_ids[k]] \
                        = np.zeros([batch_size, unroll_steps], dtype=np.int64)
        else:
            feed_dict = {}
            for k in [0, 1]:
                for model in models:
                    feed_dict[model.tokens_characters[k]] \
        ...
        if bidirectional:
            if not char_inputs:
                for k in [0, 1]:
                    for model in models:
                        feed_dict[model.token_ids_reverse[k]] \
                            = np.zeros([batch_size, unroll_steps],
                            dtype=np.int64)
            else:
                for k in [0, 1]:
                    for model in models:
